...
function calc_a3t(diameter){
  return ((10 + 5 * Math.cos(45*Math.PI/180)) * diameter);
}
function calc_a4c(diameter){
  return 5 * diameter;
}
function calc_a4t(diameter){
  return ((5 + 5 * Math.sin(45*Math.PI/180)) * diameter);
}
function calc_a1(diameter){
  return 0.7 * ((5 + 7 * Math.cos(45*Math.PI/180)) * diameter);
}
function calc_a2(diameter){
  return 0.7 * 5 * diameter;
}
...
calculate_button.addEventListener('click', () => {
  ...
  d_value = parseFloat(t_element.value);
...
const latex_condition1_element = document.getElementById("latex_condition1");
const latex_r1d_element = document.getElementById("latex_r1d");
const latex_condition2_element = document.getElementById("latex_condition2");
...
function update_latex() {
  ...
  latex_r1d_element.innerHTML = "$$ R_{1,d}=\\frac{ 2 * 5 * R_{1} * k_{mod}}{\\displaystyle{Y_{m}}} = \\frac{ 2 * 5 * " + r1_value + " * " + kmod_value +"}{\\displaystyle{" + 1.3 + "} } = {" + r1d_result.toFixed(2) + "} $$";
  ...
  latex_a3c_element.innerHTML = "$$ a_{3,c}={10 * d} = { 10 * " + d_value + " } = {" + a3c_result.toFixed(2) + "} $$";

  latex_a3t_element.innerHTML = "$$ a_{3,t}={ 10 + 5 * \\cos(45) * d} = { 10 + 5 * \\cos(45) * {" + d_value +"}} = {" + a3t_result.toFixed(2) + "} $$";

  latex_a4c_element.innerHTML = "$$ a_{4,c}={5 * d} = { 5 * " + d_value + " } = {" + a4c_result.toFixed(2) + "} $$";

  latex_a4t_element.innerHTML = "$$ a_{4,t}={ 5 + 5 * \\sin(45) * d} = { 10 + 5 * \\sin(45) * {" + d_value +"}} = {" + a4t_result.toFixed(2) + "} $$";

  latex_a1_element.innerHTML = "$$ a_{1}={ 0.7 * (( 5 + 7 * \\cos(45)) * d} = { 0.7 * (( 5 + 7 * \\cos(45) * {" + d_value +"}} = {" + a1_result.toFixed(2) + "} $$";

  latex_a2_element.innerHTML = "$$ a_{2}={0.7 * 5 * d} = { 0.7 * 5 * " + d_value + " } = {" + a2_result.toFixed(2) + "} $$";
